...
import org.trustedanalytics.atk.plugins.pregel.{ LoopyBeliefPropagationMessageSender, LoopyBeliefPropagationVertexProgram }
...
class MalformedInputTest extends FlatSpec with Matchers with TestingSparkContextFlatSpec {
  ...
  "BP Runner" should "throw an illegal argument exception when vertices have different prior state space sizes " in new BPTest {
    ...
    val exception = intercept[SparkException] {
      val (verticesOut, edgesOut, log) = PregelAlgorithm.run(verticesIn, edgesIn, args)(LoopyBeliefPropagationVertexProgram.vertexProgram, LoopyBeliefPropagationMessageSender.sendMessage)
    }
  ...
  "BP Runner" should "throw a NotFoundException when the vertex does provide the request property" in new BPTest {
    ...
    val exception = intercept[Exception] {
      PregelAlgorithm.run(verticesIn, edgesIn, args)(LoopyBeliefPropagationVertexProgram.vertexProgram, LoopyBeliefPropagationMessageSender.sendMessage)
    }
