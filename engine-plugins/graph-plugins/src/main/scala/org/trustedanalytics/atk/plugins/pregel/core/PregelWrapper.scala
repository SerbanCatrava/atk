...
class PregelWrapper(val maxIterations: Int,
   ...
   */
  def run(graph: Graph[VertexState, Double])(vertexProgram: (VertexId, VertexState, Map[Long, Vector[Double]]) => VertexState,
    ...
    Pregel(graph,
      ...
      activeDirection = EdgeDirection.Either)(vertexProgram, msgSender, mergeMsg)
